...
@app.post("/")
def root(request: Request, all_coins: List[Dict] = Depends(helpers.get_all_ticker_data),
         search_coin: Optional[str] = Form(None)):
    return_coins = []
    if search_coin == None:
...
@app.get("/wallet")
def get_coins(request: Request, clicked_coin: Optional[str] = Query(None),
              all_coins: List[Dict] = Depends(helpers.get_all_ticker_data), db: Session = Depends(get_db),
...
@app.post("/wallet")
def add_coin(request: Request, search_coin: str = Form(None),
             all_coins: List[Dict] = Depends(helpers.get_all_ticker_data), amount: int = Form(None),
             coin_name: str = Form(None), db: Session = Depends(get_db), user: schemas.User = Depends(manager)):
    user_coins = crud.get_coins_by_user_id(db=db, id=user.id)
    coin_dict = helpers.create_coin_dict(user_coins)
    return_coins = []
    ...
    added = crud.add_coin(
        db=db, coin=schemas.CoinCreate(coin_name=coin_name.lower(), amount=amount, purchase_price=purchase_price),
        id=user.id)
    if not added:
        template_dict["all_coins"] = return_coins
...
def delete_coin(id: str = Path(...), db: Session = Depends(get_db), gain_loss: float = Query(.01),
                user: schemas.User = Depends(manager)):
    print(gain_loss)
